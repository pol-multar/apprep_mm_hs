package fr.unice.polytech.si4.apprep.tp6.exo1.q2;
...
public class CopyOfProdCons implements javax.jms.MessageListener{
    ...
    private void configurerConsommateur() throws JMSException, NamingException{
        ...
        receiveSession = connect.createSession(true,-1);//n'importe quelle valeur pour 2nd param car ignoré  
	...
	public static void main(String[] args) {
		try {
			(new fr.unice.polytech.si4.apprep.tp6.exo1.q1.CopyOfProdCons()).configurer();
	...
	int nbrecus=0;
	boolean switchTransacCommit=true;
	@Override
	public void onMessage(Message message) {
		...
		try {
			...
			nbrecus++; // un message de plus recu, mais pas encore atteint la fin de la transac.
			if (nbrecus==5 && !switchTransacCommit){
				System.out.println("Simulons un probleme durant l'execution de onMessage entrainant l'annulation de la transac");
				receiveSession.rollback();
				nbrecus=0;
				switchTransacCommit=true;//pour changer de comportement et demarrer une transaction qui se terminera avec succes
			} else if (nbrecus==5) {
				System.out.println("On va committer la reception");
				receiveSession.commit();
				switchTransacCommit=false;
				nbrecus=0; // redemarrer une nouvelle transaction
			}
		} catch (JMSException e){ 
		// TODO Auto-generated catch block
		e.printStackTrace();
